...
class pdf extends TCPDF
{
	public function Header() {
		...
		$this->Cell(180, 5, 'Daily Confirmed Order Report', 0, 1, 'C');
	...
	public function Footer() {
		...
		$this->Ln(3);
		$this->Cell(180, 3, '_____________________________________________________________________________________', 0, 1, 'C');
		$this->Cell(180, 3, ' - Report of daily confirmed order from Team DigiMart -', 0, 1, 'C');
...
$pdf->SetTitle('Daily Confirmed Order Report');
$pdf->SetSubject('');
$pdf->SetKeywords('');
...
$query2 = "SELECT o.`id` AS 'orderId', o.*, c.*, m.* FROM `order_product` o, `customer` c, `customer_mail_info` m WHERE o.`is_canceled` = 0 AND o.`is_deleted` = 0 AND o.`is_posted` = 0 AND c.`id` = o.`customer_id` AND m.`customer_id` = c.`id` AND m.`is_default` = 1 AND o.`date_time` <= '".date("Y-m-j H:i:s", strtotime('-1 days'))."' ORDER BY o.`date_time` ASC";
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->SetTextColor(0,0,0);
    $pdf->SetFont('times', '', 12);
    $pdf->Cell(40, 4, "Product ID:", 0, 0, 'L');
    $pdf->SetFont('Courier', 'B', 12);
    $pdf->SetTextColor(221, 18, 61);
    $pdf->Cell(60, 4, $row['product_id'], 0, 0, 'L');
    $pdf->SetTextColor(0,0,0);
    $pdf->SetFont('times', '', 12);
    $pdf->Cell(25, 4, "Quantity:", 0, 0, 'L');
    $pdf->SetFont('Courier', 'B', 12);
    $pdf->SetTextColor(221, 18, 61);
    $pdf->Cell(25, 4, $row['quantity'], 0, 1, 'L');
    $pdf->SetFont('Courier', '', 10);
    $pdf->SetTextColor(0,0,0);
    ...
    $pdf->SetFont('times', '', 12);
    $pdf->SetTextColor(0,0,0);
    $pdf->Cell(40, 4, "Unit Price:", 0, 0, 'L');
    $pdf->SetFont('Courier', 'B', 12);
    $pdf->SetTextColor(221, 18, 61);
    $pdf->Cell(115, 4, "LKR ".$row['unit_price'], 0, 1, 'L');
...
$pdf->Output('Daily confirmed order report.pdf', 'I');
