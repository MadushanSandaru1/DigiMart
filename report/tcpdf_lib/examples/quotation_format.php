...
session_start();
	...
	if(isset($_SESSION['digimart_current_user_id'])) {
        $sql = "SELECT p.id, p.`name` FROM `quotation` c, `product` p WHERE c.`product_id` = p.`id` AND c.`customer_id` = '{$_SESSION['digimart_current_user_id']}'";

        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)) {
                $cartItemName[$row['id']] = $row['name'];
            }
        } else {
            $cartItemName = [];
        }
...
class pdf extends TCPDF
{
	public function Header() {
		...
		$this->Cell(180, 5, 'Quotation Report', 0, 1, 'C');
	...
	public function Footer() {
		...
		$this->Ln(5);
		$this->Cell(180, 3, '_____________________________________________________________________________________', 0, 1, 'C');
		$this->Cell(180, 3, ' - Report of product quotation from Team DigiMart -', 0, 1, 'C');
...
if(isset($_GET['itemCount'])) {
	...
	while ($i <= $itemCount) {
		...
		$pdf->MultiCell(70, 4, $cartItemName[$productId[$i-1]], 0, 'L', 0, 0, '', '', true);
